...
public abstract class LineCsvParser {
    ...
    protected Character parseLine(String line) {
        ...
        String originName = normalize(parts[6]);
        String locationName = normalize(parts[7]);
        String created = normalize(parts[8]);

        return new Character(id, name, status, species, type, gender, originName, locationName, created);
    }
